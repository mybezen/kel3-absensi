...
<div class="flex flex-col items-center min-h-screen bg-white">
    ...
    <div class="w-full p-6 text-center bg-white shadow-md">
        <!-- Existing profile display section -->
        <div class="mb-5">
            <img id="profileImage" src="{{ asset('images/Ari.jpeg') }}" alt="Profile Image" class="w-[8rem] mx-auto mb-2 rounded-full">
            <h2 id="profileName" class="text-lg poppins-semibold text-[#686D57]">Amri Ikhda</h2>
            <p class="poppins-semibold text-[#686D57]">Karyawan</p>
            <button id="editProfileButton" class="w-full bg-[#E0E5B6] text-[#686D57] hover:bg-[#686D57] hover:text-[#E0E5B6] py-2 rounded-xl mt-3 poppins-bold">Edit Profil</button>
        </div>

        <!-- Profile editing form (hidden initially) -->
        <div id="editProfileForm" class="mb-5 hidden">
            <form id="profileForm">
                <div class="mb-4">
                    <label for="name" class="block text-[#686D57] text-sm poppins-semibold mb-2">Nama:</label>
                    <input type="text" id="nameInput" name="name" class="w-full p-2 border border-[#686D57] rounded-lg" placeholder="Masukkan Nama">
                </div>
                <div class="mb-4">
                    <label for="profileImage" class="block text-[#686D57] text-sm poppins-semibold mb-2">Foto Profil:</label>
                    <input type="file" id="profileImageInput" name="profileImage" class="w-full p-2 border border-[#686D57] rounded-lg">
                </div>
                <button type="submit" class="bg-[#E0E5B6] text-[#686D57] hover:bg-[#686D57] hover:text-[#E0E5B6] w-full py-2 rounded-xl mt-3 poppins-bold">Simpan Perubahan</button>
            </form>
        </div>
        ...
        <div id="attendanceContainer">
            <a href="/prform" id="attendanceLink">
                <button id="attendanceButton" class="bg-[#E0E5B6] w-full text-[#686D57] py-2 rounded-xl mb-5 poppins-bold hover:bg-[#686D57] hover:text-[#E0E5B6]">Presensi Masuk</button>
            </a>
        </div>
        <div class="mt-4 grid grid-cols-3 gap-4 mb-5 poppins-regular">
            <a href="/report">
                <div class="text-center rounded-2xl bg-white hover:bg-[#E0E5B6] p-2 border-4 border-[#E0E5B6]">
                    <!-- Icon Laporan Tugas -->
                    <svg class="w-8 h-8 mx-auto mb-1 text-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
...
<script>
    ...
    document.getElementById('profileForm').addEventListener('submit', function(e) {
        ...
        var profileImageInput = document.getElementById('profileImageInput').files[0];
        if (nameInput) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        var hasCheckedIn = localStorage.getItem('hasCheckedIn') === 'true';
        updateAttendanceButton(hasCheckedIn);

        document.getElementById('attendanceButton').addEventListener('click', function() {
            hasCheckedIn = !hasCheckedIn;
            localStorage.setItem('hasCheckedIn', hasCheckedIn);
            updateAttendanceButton(hasCheckedIn);
        });
    });

    function updateAttendanceButton(hasCheckedIn) {
        var button = document.getElementById('attendanceButton');
        var link = document.getElementById('attendanceLink');
        if (hasCheckedIn) {
            button.textContent = 'Presensi Keluar';
            link.setAttribute('href', '/prform');
        } else {
            button.textContent = 'Presensi Masuk';
            link.setAttribute('href', '/prform');
        }
    }
</script>
